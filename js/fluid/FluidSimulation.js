...
export class FluidSimulation {
  constructor(canvas, config) {
    this.config = config;
    this.enabled = true; // master on/off (paused when tab hidden)
    this.suppressSplat = false; // true while the smudge section owns the cursor
  ...
  _loop() {
    ...
    const tick = () => {
      ...
      // paused (hidden tab): skip GPU work but keep the loop alive
      if (!this.enabled) {
        this.mouse.moved = false;
        return;
      }

      // splat unless the smudge section is currently suppressing us
      if (this.mouse.moved && !this.suppressSplat) {
        this._splat(
          this.mouse.x,
          this.mouse.y,
          this.mouse.velocityX,
      ...
      this.mouse.moved = false; // always clear, so motion under the
      // smudge band doesn't queue a splat that fires on leave
